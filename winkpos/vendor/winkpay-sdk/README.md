...
dependencies {
    implementation("com.wink:winkpay-sdk:1.7.14")
}
```
...
| `CAMERA_UNAVAILABLE` | The face camera could not be opened, or opened but never delivered a frame, and the session's automatic recovery did not help. **Not the camera at all** if logcat shows `FaceCaptureSession: Camera init failed … MlKitContext has not been initialized`: the host process was started in Android's direct-boot phase (a `directBootAware` `LOCKED_BOOT_COMPLETED` receiver on a file-encrypted device), where ML Kit's init provider is not installed, and it stayed that way after unlock. From 1.7.14 the SDK initialises ML Kit itself before creating the detector; hosts should still start their boot service from `BOOT_COMPLETED`, not `LOCKED_BOOT_COMPLETED`. Recovery (1.7.13): CameraX's own reopen is left to run first — an in-use / max-cameras / recoverable error that arrives while the camera is still OPENING or PENDING_OPEN only extends the first-frame watchdog (6 s, up to 12 s per attempt); only a CLOSED/CLOSING error, a bind that throws, or a watchdog expiry triggers the session's unbind + rebind (three attempts ~750 ms apart). 1.7.12 unbound on the first error event, which aborted CameraX's retry and turned a camera still closing from the previous session into a hard failure ~2.3 s later. Typical causes: another client holds the camera, the device HAL is still tearing down the previous session (seen on PAX A380 when a capture is cancelled and relaunched within ~1.5 s), or the camera is disabled by policy. With `returnOnFailure` the host gets this code; otherwise the SDK shows its retry screen. Before 1.7.12 this case was silent: the "Initializing Camera…" overlay stayed until the 30 s capture timeout, whose check-in-mode cancel path re-mounted the capture and bound the same camera again. |
